package com.advikonsignagekeyboardplayer.api_manager;
...
public class DownloadService extends Service {
    ...
    ArrayList<Announcement> announcementsToBeDownloaded = new ArrayList<>();
    ...
    int downloadingAnnouncementAtIndex = 0;
...
//                File[] pathsss = getExternalFilesDirs(getApplicationContext(),null);
            ...
            String filePath1 = gblStorage.getAbsolutePath() + File.separator + songsToBeDownloaded.
                    get(downloadingFileAtIndex).getTitle_Id() + Constants.TAG_FILE_EXTENSION_MP3;
                        ...
                        public void onCompleted(Exception e, File result) {
                            ...
                            if (e != null) {
                                ...
                                handler.postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(DownloadService.this, "Downloading failed for " + fileName, Toast.LENGTH_SHORT).show();
                                    }
                                }, 1000);
                                startDownloadingSongs();
                                return;
...
//                   int progress = (int) (downloaded / total * 100.0);
    ...
    private void prepareForDownloadingAds(){
        ...
        ArrayList<Announcement> announcementArrayList = announcementManager.getAnnouncementsToBeDownloaded();
        ...
        if (advertisementsArrayList != null && advertisementsArrayList.size() > 0){
            ...
            startDownloadingAds();
        } else if (announcementArrayList != null && announcementArrayList.size() > 0) {
            announcementsToBeDownloaded.addAll(announcementArrayList);
            startDownloadingAnnouncements();
        } else {
...
//                   int progress = (int) (downloaded / total * 100.0);
    ...
    private void startDownloadingAnnouncements(){

        try {

            if (announcementsToBeDownloaded.size() > 0){

                String applicationDirectory = getApplicationInfo().dataDir;

                String fileURL = announcementsToBeDownloaded.
                        get(downloadingAnnouncementAtIndex).getAncFileUrl();

                String fileName = Utilities.removeSpecialCharacterFromFileName(announcementsToBeDownloaded.
                        get(downloadingAnnouncementAtIndex).getAncID());

                File announcementsFolder = new File(applicationDirectory + File.separator + Constants.ANNOUNCEMENTS_FOLDER);

                if (!announcementsFolder.exists()){
                    announcementsFolder.mkdir();
                }

                String fileExtension = FileUtil.getExtension(fileURL);

                fileName = fileName + fileExtension;

                String filePath = applicationDirectory + File.separator + Constants.ANNOUNCEMENTS_FOLDER + File.separator + fileName;

                File existingFile = new File(filePath);

                //If file already exists at download location, change the status in db only.
                if (existingFile != null && existingFile.exists()){
                    final Announcement announcement = announcementsToBeDownloaded.get(downloadingAnnouncementAtIndex);
                    announcement.setStatus_Download(1);
                    announcement.setAncFilePath(existingFile.getAbsolutePath());
                    new AnnouncementManager(DownloadService.this).announcementDownloaded(announcement);
                    EventBus.getDefault().post(new MessageEvent(announcement));
                    if (announcementsToBeDownloaded.size() - 1 > downloadingAnnouncementAtIndex){
                        downloadingAnnouncementAtIndex++;
                        startDownloadingAnnouncements();
                    } else {
                        EventBus.getDefault().post(new MessageEvent((Announcement) null));
                        sendCopyIntimationForSongsCopied(0);
                        stopSelf();
                    }
                    return;
                ...
                Ion.with(DownloadService.this).load(fileURL)
                        .progress(new ProgressCallback() {
                            ...
                            public void onProgress(long downloaded, long total) {
                                ...
                                Log.e("Anc downloaded", percentage + "%");
                        ...
                        .setCallback(new FutureCallback<File>() {
                            ...
                            public void onCompleted(Exception e, File result) {
                                ...
                                if (result != null && result.exists()){
                                    ...
                                    final Announcement announcement = announcementsToBeDownloaded.get(downloadingAnnouncementAtIndex);
                                    announcement.setStatus_Download(1);
                                    announcement.setAncFilePath(result.getAbsolutePath());
                                    new AnnouncementManager(DownloadService.this).announcementDownloaded(announcement);
