...
//            String query= "select * from"+ MySQLiteHelper.TABLE_PLAYLIST + "where"+ time +"between"+ starttime+ "and"+ endTime;
    ...
    private Playlist cursorToPlaylist(Cursor cursor) {
        ...
        playlist.setPlaylistCategory(cursor.getString(9));
        return playlist;
    }
...
//            String query= "select * from"+ MySQLiteHelper.TABLE_PLAYLIST + "where"+ time +"between"+ starttime+ "and"+ endTime;
    ...
    public int getSongsCountForPlaylistId(String playlistId){
        ...
        String query= "select count(*) from " + MySQLiteHelper.TABLE_SONGS + " where "+ MySQLiteHelper.COLUMN_SP_PLAYLIST_ID+" = "+ playlistId;
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            cursor.moveToFirst();
            String countString = cursor.getString(cursor.getColumnIndex(cursor.getColumnName(0)));

            if (countString != null && countString != "") {

                try {
                    Integer newCountPadded = Integer.parseInt(countString);
                    count = newCountPadded;
                } catch (Exception e){
                    e.printStackTrace();
                }
            }
